...
const Projects = () => {
  const { data: getMyProjectsList, isPending } = useFetchData({
    key: "getMyProjects",
    url: getMyProjects,
  });
  return (
    <>
      <div className="row">
        <div className="col-lg-10 col-12 d-flex gap-2 flex-column mb-3">
          ...
          <span className="f-16 fnt-light-color">Things I’ve built so far</span>
        </div>
        {isPending ? (
          ...
          getMyProjectsList?.map((project) => (
            <div
              key={project._id}
              className="col-lg-5 col-12 card m-lg-3 m-2 mb-lg-0 mb-3  "
            >
